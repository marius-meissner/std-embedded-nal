...
impl TcpSocket {
    fn get_running(&mut self) -> std::io::Result<&mut TcpStream> {
        match &mut self.state {
            ...
            _ => Err(std::io::Error::new(std::io::ErrorKind::NotConnected, OutOfOrder))
...
impl embedded_nal::TcpStack for crate::Stack {
    ...
    fn open(&self, mode: embedded_nal::Mode) -> std::io::Result<TcpSocket> {
        Ok(TcpSocket { state: SocketState::Building(mode) })
    }
    fn connect(&self, socket: TcpSocket, remote: embedded_nal::SocketAddr) -> std::io::Result<TcpSocket> {
        ...
        match mode {
            ...
            embedded_nal::Mode::Timeout(millis) => {
                socket.set_nonblocking(false)?;
                socket.set_read_timeout(Some(std::time::Duration::from_millis((*millis).into())))?;
                socket.set_write_timeout(Some(std::time::Duration::from_millis((*millis).into())))?;
            }
        }
    ...
    fn read(&self, socket: &mut TcpSocket, buffer: &mut [u8]) -> Result<usize, nb::Error<std::io::Error>> {
        let socket = socket.get_running()?;
