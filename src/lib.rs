...
/// The operating system's UDP stack, implementing ``embedded_nal::UdpStack``
///
/// This is most easily accessed using the static ``udp`` instance.
pub struct Udp;
...
impl embedded_nal::UdpStack for Udp {
    type UdpSocket = UdpSocket;
    type Error = std::io::Error;

    fn open(&self, remote: embedded_nal::SocketAddr, mode: embedded_nal::Mode) -> std::io::Result<UdpSocket> {
        let any = match remote {
            embedded_nal::SocketAddr::V4(_) => SocketAddr::new(IpAddr::V4(Ipv4Addr::UNSPECIFIED), 0),
            embedded_nal::SocketAddr::V6(_) => SocketAddr::new(IpAddr::V6(Ipv6Addr::UNSPECIFIED), 0),
        };
    ...
    fn write(&self, socket: &mut UdpSocket, buffer: &[u8]) -> Result<(), nb::Error<std::io::Error>> {
        socket.send(buffer)
            .map(|s| if s == buffer.len() {
                ()
            } else {
    ...
    fn read(&self, socket: &mut UdpSocket, buffer: &mut [u8]) -> Result<usize, nb::Error<std::io::Error>> {
        socket.recv(buffer)
            .map_err(std_to_nal_error)
    }
